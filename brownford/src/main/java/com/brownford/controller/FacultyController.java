    ...
    public String facultyWorkload(
        ...
        if (principal != null) {
            ...
            if (facultyOpt.isPresent()) {
                ...
                java.util.List<String> schoolYears = assignments.stream().map(a -> a.getSchoolYear()).distinct().sorted((a, b) -> b.compareTo(a)).collect(java.util.stream.Collectors.toList());
                java.util.List<String> semesters = assignments.stream().map(a -> a.getSemester()).filter(s -> s.equals("First") || s.equals("Second")).distinct().sorted().collect(java.util.stream.Collectors.toList());
                // Default to latest school year and first semester if not provided
                if (sy == null && !schoolYears.isEmpty()) sy = schoolYears.get(0);
                if (sem == null && !semesters.isEmpty()) sem = semesters.get(0);
                final String selectedSy = sy;
                final String selectedSem = sem;
                // Filter assignments by selected school year and semester
                assignments = assignments.stream()
                    .filter(a -> (selectedSy == null || a.getSchoolYear().equals(selectedSy)) && (selectedSem == null || a.getSemester().equals(selectedSem)))
                    .collect(java.util.stream.Collectors.toList());
                int rowNum = 1;
                for (FacultyAssignment assignment : assignments) {
...
class FacultyWorkloadRestController {
    ...
    public Map<String, Object> getWorkloadFilters(Principal principal) {
        ...
        var schoolYears = assignments.stream().map(a -> a.getSchoolYear()).distinct().sorted().collect(Collectors.toList());
        var semesters = assignments.stream().map(a -> a.getSemester()).distinct().sorted().collect(Collectors.toList());
        // Format for frontend [{value, label}]
        result.put("schoolYears", schoolYears.stream().map(y -> Map.of("value", y, "label", y)).collect(Collectors.toList()));
        result.put("semesters", semesters.stream().filter(s -> !"Summer".equalsIgnoreCase(s)).map(s -> Map.of("value", s, "label", s)).collect(Collectors.toList()));
        return result;
    }
}
...
class FacultyProfileRestController {
    ...
    public Map<String, Object> getProfileInfo(Principal principal) {
        ...
        result.put("specialization", faculty.getSpecialization());
        result.put("position", faculty.getPosition());
        result.put("highestDegree", faculty.getHighestDegree());
        result.put("employmentStatus", faculty.getEmploymentStatus());
        result.put("officeLocation", faculty.getOfficeLocation());
        result.put("officeHours", faculty.getOfficeHours());
        result.put("emergencyContact", faculty.getEmergencyContact());
        result.put("researchInterests", faculty.getResearchInterests());
        result.put("publications", faculty.getPublications());
        return result;
    }
    ...
    public Map<String, Object> updateProfileInfo(@RequestBody Map<String, Object> payload, Principal principal) {
        ...
        String username = principal.getName();
        var userOpt = userRepository.findByUsername(username);
        if (userOpt.isEmpty()) { result.put("success", false); return result; }
        var user = userOpt.get();
        var facultyOpt = facultyRepository.findByUser(user);
        if (facultyOpt.isEmpty()) { result.put("success", false); return result; }
        var faculty = facultyOpt.get();
        // Update fields
        if (payload.containsKey("gender")) faculty.setGender((String) payload.get("gender"));
        if (payload.containsKey("dateOfBirth")) faculty.setDateOfBirth((String) payload.get("dateOfBirth"));
        if (payload.containsKey("mobileNumber")) faculty.setMobileNumber((String) payload.get("mobileNumber"));
        if (payload.containsKey("address")) faculty.setAddress((String) payload.get("address"));
        if (payload.containsKey("specialization")) faculty.setSpecialization((String) payload.get("specialization"));
        if (payload.containsKey("position")) faculty.setPosition((String) payload.get("position"));
        if (payload.containsKey("highestDegree")) faculty.setHighestDegree((String) payload.get("highestDegree"));
        if (payload.containsKey("employmentStatus")) faculty.setEmploymentStatus((String) payload.get("employmentStatus"));
        if (payload.containsKey("officeLocation")) faculty.setOfficeLocation((String) payload.get("officeLocation"));
        if (payload.containsKey("officeHours")) faculty.setOfficeHours((String) payload.get("officeHours"));
        if (payload.containsKey("emergencyContact")) faculty.setEmergencyContact((String) payload.get("emergencyContact"));
        if (payload.containsKey("researchInterests")) faculty.setResearchInterests((String) payload.get("researchInterests"));
        if (payload.containsKey("publications")) faculty.setPublications((String) payload.get("publications"));
    ...
    public Map<String, Object> changePassword(@RequestBody Map<String, String> payload, Principal principal) {
        Map<String, Object> result = new HashMap<>();
        if (principal == null) { result.put("success", false); result.put("message", "Not authenticated"); return result; }
        String username = principal.getName();
        var userOpt = userRepository.findByUsername(username);
